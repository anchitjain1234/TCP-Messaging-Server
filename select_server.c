...
void messageprocess(char *mess,int *clients,int groupinfo[][MAX_CLIENTS],int *cligroup,int fd)
{
    int i,j,len;
    len=strlen(mess);
...
int main(int argc,char *argv[])
{
    ...
    char buff[MAXLINE];
    int serverfd,clientfd,clilen,sockfd,clients[MAX_CLIENTS],groupinfo[MAX_GROUPS][MAX_CLIENTS],cligroup[1024];
    ...
    while(true)
    {
        ...
        if(FD_ISSET(serverfd,&read_set))
        {
            ...
            clientfd=accept(serverfd,(struct sockaddr *)&client_address,(socklen_t *)&clilen);
